# Importador MercadoPago CSV — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Agregar un modal en el tab Egresos que parsee el CSV de MercadoPago y cargue cada transacción negativa como un egreso individual con detección de duplicados por REFERENCE_ID.

**Architecture:** Nuevo módulo `modules/mp-import.js` totalmente auto-contenido. Tiene su propio modal (no reutiliza el modal genérico). Las funciones helpers del modal se exponen a `window` dentro de `renderMPImportModal()` para que los onclick del HTML dinámico funcionen tanto en bundle como en ES module mode. Los IDs se generan con un generador local que opera sobre `d.egresos` en memoria para evitar colisiones en imports batch.

**Tech Stack:** Vanilla JS, sin dependencias externas. Usa `ld()/sd()` para storage, `nEId()`-equivalente local para IDs batch, CSS variables del sistema (`var(--bg)`, `var(--er)`, etc.).
...
|---|---|---|
| CREAR | `modules/mp-import.js` | Módulo completo: parser CSV, modal, confirmación |
| MODIFICAR | `modules/egresos.js` | Badge "MP" en historial para egresos importados |
| MODIFICAR | `index.html` | Botón "⬆ Importar MP" en la sección Historial de Egresos |
| MODIFICAR | `main.js` | Import + window exposure de `renderMPImportModal` |
| MODIFICAR | `build.ps1` | Agregar `modules/mp-import.js` al array `$files` |

---

## Task 1 — Crear `modules/mp-import.js`

**Files:**
- Create: `modules/mp-import.js`

- [ ] **Step 1.1: Crear el archivo con el módulo completo**

Crear `modules/mp-import.js` con este contenido exacto:
...
- [ ] **Step 1.2: Verificar que el archivo fue creado**

```
ls modules/mp-import.js
```
Esperado: el archivo existe.

---

## Task 2 — Badge "MP" en historial (`modules/egresos.js`)

**Files:**
- Modify: `modules/egresos.js` línea 68

- [ ] **Step 2.1: Agregar badge MP en la línea del egreso en `rEH()`**

En `modules/egresos.js`, en la función `rEH()`, buscar la línea:
...
- [ ] **Step 2.2: Commit parcial**

```bash
git add modules/egresos.js
git commit -m "feat: badge MP en historial de egresos importados"
```

---

## Task 3 — Botón "⬆ Importar MP" en `index.html`

**Files:**
- Modify: `index.html` — sección Historial de Egresos (~línea 363)

- [ ] **Step 3.1: Agregar el botón en la barra de acciones del historial**

En `index.html`, buscar exactamente:

```html
    <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">
      <button class="btn btn-d" onclick="anularEgresoByIdModal()" style="padding:0 10px;font-size:9px">🗑 Anular ID</button>
      <button class="btn btn-d" onclick="limpiarEgresos()" style="padding:0 10px;font-size:9px">⚠ Limpiar todo</button>
    </div>
```
...
```html
    <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">
      <button class="btn btn-d" onclick="anularEgresoByIdModal()" style="padding:0 10px;font-size:9px">🗑 Anular ID</button>
      <button class="btn btn-d" onclick="limpiarEgresos()" style="padding:0 10px;font-size:9px">⚠ Limpiar todo</button>
      <button class="btn btn-p" onclick="renderMPImportModal()" style="padding:0 10px;font-size:9px;margin-left:auto">⬆ Importar MP</button>
    </div>
```

- [ ] **Step 3.2: Commit parcial**

```bash
git add index.html
git commit -m "feat: botón Importar MP en tab Egresos"
```

---

## Task 4 — Import y window exposure en `main.js`

**Files:**
- Modify: `main.js` — línea 12 (bloque de imports) y línea ~151 (bloque de egresos en Object.assign)

- [ ] **Step 4.1: Agregar el import de `renderMPImportModal`**

En `main.js`, buscar la línea:
...
- [ ] **Step 4.2: Agregar `renderMPImportModal` al Object.assign de window**
...
- [ ] **Step 4.3: Commit parcial**

```bash
git add main.js
git commit -m "feat: import y window exposure de renderMPImportModal"
```

---

## Task 5 — Agregar `mp-import.js` al bundle

**Files:**
- Modify: `build.ps1` — array `$files`

- [ ] **Step 5.1: Insertar `modules/mp-import.js` después de `modules/egresos.js`**
...
- [ ] **Step 5.2: Commit parcial**

```bash
git add build.ps1
git commit -m "build: agregar mp-import.js al bundle"
```

---

## Task 6 — Build, commit final y verificación

**Files:**
- Modify: `bundle.js` (generado), `modules/mp-import.js` (ya creado)

- [ ] **Step 6.1: Agregar mp-import.js al staging y correr el build**

```powershell
git add modules/mp-import.js
.\build.bat
```

Esperado: output del build muestra `[+] modules/mp-import.js` y termina con "Listo."

- [ ] **Step 6.2: Verificar que bundle.js incluye el módulo**

```powershell
Select-String "mp-import" bundle.js | Select-Object -First 3
```

Esperado: al menos una línea con `// ===== modules/mp-import.js =====`

- [ ] **Step 6.3: Commit final**

```bash
git add bundle.js modules/mp-import.js
git commit -m "feat: importador MercadoPago CSV — modal con preview, edición inline y dedup por REFERENCE_ID"
```

- [ ] **Step 6.4: Prueba manual en el browser**

1. Abrir `index.html` (doble click) o correr `serve.bat`
2. Ir al tab **Egresos**
3. En la sección "Historial de Egresos" verificar que aparece el botón **⬆ Importar MP** alineado a la derecha
4. Clickear el botón → debe abrirse el modal con la drop zone
5. Arrastrar `MP/account_statement-aa82bbfa-b210-4541-af45-322a979b8b77.csv` al modal
6. Verificar que aparece la tabla con los egresos (montos negativos del CSV), positivos excluidos
7. Editar el concepto de alguna fila (ej: cambiar "Pago con QR VEA" a "Super VEA 🛒")
8. Desseleccionar alguna fila con el checkbox
9. Click **Importar N egresos →**
10. Verificar toast de confirmación y que la lista de egresos se actualiza
11. Los egresos importados deben mostrar el badge **MP** al lado del concepto
12. Abrir el modal de nuevo y cargar el mismo CSV → las filas ya importadas deben aparecer con "ya importado" y checkbox desactivado (dedup funcionando)

- [ ] **Step 6.5: Push a GitHub**

```powershell
$TOKEN = (Get-Content "token github.txt")[1].Trim()
git push "https://$TOKEN@github.com/fungiabduction-ui/cashflow.git" clean-main:main
```

Esperado: `clean-main -> main` confirmado.

---

## Notas para el implementador

- **`_nEIdLocal(d, mes)`** opera sobre `d.egresos` (en memoria), NO sobre `gE()`. Esto es crítico para que un batch de varios egresos del mismo mes no genere IDs duplicados.
- **`mpRefId`** es un campo nuevo, opcional. Los egresos sin él (todos los manuales) tienen `mpRefId: undefined`, lo que es retrocompatible.
- **`addMon(mes, 0)`** devuelve `"MM/YYYY"` (ej: `"06/2026"`). Eso es el valor correcto para el campo `finaliza` en egresos de una sola cuota.
- En bundle mode, las funciones `_mpHandleDrop`, `cerrarMPModal`, etc. son globales automáticamente. En ES module mode (serve.bat), se exponen a `window` dentro de `renderMPImportModal()` antes de insertar el modal en el DOM.
- La rama local es `clean-main` pero el remote es `main` — el push debe ser `clean-main:main`.
