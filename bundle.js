...
function backupFingerprint(dataObj){
  const clean={...dataObj};
...
async function ghDownloadBackup(path){
  const cfg=ghCfg();
  if(!cfg.token||!cfg.repo){sN('Configura GitHub primero',true);return;}
  try{
...
async function ghRestoreBackup(path){
...
function renderWAText(){
  ...
  function waEmoji(i,n){
    if(i===0)return'⚡';
    const topStart=1+Math.floor((n-1)*2/3);
    return i>=topStart?'🚀':'🔥';
  }
